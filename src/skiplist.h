...
inline bool key_at_matches(const Key arr[], int idx, const Key& target) {
    return idx >= 0 && idx < ARR_SIZE && compare_(arr[idx], target) == 0;
}
        ...
        // Case 1: There is no node in list and need to make a new node
            ...
            if (prev_[level]->forward->N_key != ARR_SIZE) { // Case 2-1: Forward node has a room
                if (level == 0) { // Case 2-1-1: Insert into H0
                    ...
                    if (key_at_matches(prev_[level]->forward->keys, idx, key)) {
                        break;
                        ...
                        if (idx == -1) {
                            for (int i = level+1; i < GetMaxHeight(); i++) {
                                if (prev_[i] != nullptr) {
                                    ...
                                    if (key_at_matches(prev_[i]->forward->keys, idx, update_key)) {
                    ...
                    if (key_at_matches(prev_[level]->forward->keys, idx, prev_[level-1]->forward->keys[0])) {
                        break;
                    } else if (prev_[level]->forward->keys[idx+1] == 0) {
            ...
            if (prev_[level]->N_key != ARR_SIZE) { // Case 3-1: prev_ node has a room, so we insert into that node
                if (level == 0) { // Case 3-1-1: Insert into H0
                    ...
                    if (key_at_matches(prev_[level]->keys, idx, key)) {
                        stop_flag++;
                    } else {
                    ...
                    if (key_at_matches(prev_[level]->keys, idx, prev_[level-1]->keys[0])) {
                        stop_flag++;
                    } else {
                        if (prev_[level]->keys[idx+1] == 0) {
                                ...
                                if (idx == ARR_SIZE / 2 - 1) {
                                    ...
                                    if (idx == -1) {
                                        for (int update = level+1; update < GetMaxHeight(); update++) {
                                            ...
                                            if (key_at_matches(prev_[update]->forward->keys, idx, update_key)) {
            ...
            if (x->N_key > 1) {
                ...
                for (int i = level+1; i < GetMaxHeight(); i++) {
                    if (prev_[i] != nullptr) {
                        ...
                        if (key_at_matches(prev_[i]->keys, idx, del_key)) {
